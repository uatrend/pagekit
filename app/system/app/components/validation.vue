    ...
    <ValidationObserver slim>
        <ValidationProvider :name="$attrs.name" :rules="rules" v-slot="{ errors }" slim>
            <div v-if="!get('type')" :class="get('containerClass') ? get('containerClass') : 'uk-form-controls'">
                <input v-if="!get('tag')" :id="$attrs.id" :name="$attrs.name" :type="$attrs.type" :placeholder="$attrs.placeholder | trans" :class="[get('class'), {'uk-form-danger': get('danger') && errors[0]}]" v-model="innerValue" />
                <textarea v-else-if="get('tag') === 'textarea'" :id="$attrs.id" :name="$attrs.name" :type="$attrs.type" :placeholder="$attrs.placeholder | trans" :rows="$attrs.rows" :class="[get('class'), {'uk-form-danger': get('danger') && errors[0]}]" v-model="innerValue"></textarea>
            ...
            <div v-else-if="get('type') === 'icon'" :class="get('containerClass')">
                <div class="uk-inline">
                    ...
                    <input :id="$attrs.id" :name="$attrs.name" :type="$attrs.type" :placeholder="$attrs.placeholder | trans" :class="[get('class'), {'uk-form-danger': get('danger') && errors[0]}]" v-model="innerValue" />
                </div>
                <div v-if="errors[0]" class="uk-text-meta uk-text-danger">{{ $attrs.message | trans }}</div>
            </div>
        </ValidationProvider>
